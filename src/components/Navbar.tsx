...
const Navbar = () => {
  ...
  return (
    <section className="">
                ...
                <Link to="/">
                  <AnimatePresence mode="wait">
                    <motion.img
                      key={currentLogo}
                      src={currentLogo}
                      className="h-20 w-40 object-contain ml-[40px]"
                      alt="Logo"
            ...
            <div className="flex flex-wrap mr-7 items-center">
              <div className="w-auto hidden xl:block">
                <ul className="flex items-center mr-8">
                  {navLinks.map((link) => (
                    <li key={link.path} className="mr-14 font-medium hover:text-gray-900 tracking-tight">
                      <Link
                        to={link.path}
      ...
      {/* Mobile Navigation */}
      <div
        className={`fixed top-0 left-0 bottom-0 w-4/5 sm:max-w-xs z-50 transition-transform transform ${mobileNavOpen ? 'translate-x-0' : '-translate-x-full'}`}
        ...
        <nav className="relative z-10 px-9 pt-8 h-full overflow-y-auto" style={{ backgroundColor: '#e5e7eb' }}>
          <div className="flex flex-wrap justify-between h-full">
            ...
            <div className="flex flex-col justify-center py-16 w-full">
              <ul>
                {navLinks.map((link) => (
                  <li key={link.path} className="mb-6 font-medium hover:text-gray-900 tracking-tight">
                ...
                {user ? (
                  ...
                  <li className="font-medium mb-6 hover:text-gray-900 tracking-tight">
                    <Link to="/login">Sign In</Link>
                  </li>
                )}
              </ul>
            ...
            {!user && (
              <div className="flex flex-col justify-end w-full pb-20">
                <Link
                  className="inline-block hover:shadow-2xl px-5 py-3 text-white font-semibold text-center tracking-tight bg-gradient-to-r from-gray-400 via-gray-600 to-blue-800 hover:bg-indigo-600 rounded-lg transition duration-200"
                  to="/book-consultation"
                >
                  {texts[currentTextIndex]}
                </Link>
